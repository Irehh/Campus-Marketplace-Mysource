...
exports.getGigs = async (req, res) => {
  try {
    ...
    if (search) {
      ...
      if (where[Op.or]) {
        where[Op.or].push(
          sequelize.where(sequelize.cast(sequelize.col("skills"), "text"), { [Op.like]: `%${search}%` }),
        )
      }
    }
    ...
    const bidCounts = await Bid.findAll({
      attributes: ["gigId", [sequelize.fn("COUNT", sequelize.col("id")), "count"]],
      where: {
        gigId: {
          [Op.in]: gigIds,
        },
...
exports.updateGig = async (req, res) => {
  ...
  try {
    const { id } = req.params
    const { title, description, budget, duration, category, skills, status, replaceImages } = req.body
...
exports.cancelGig = async (req, res) => {
  ...
  try {
    ...
    await gig.update(
      {
        status: "cancelled",
        paymentStatus: gig.paymentStatus === "escrow" ? "refunded" : gig.paymentStatus,
      },
      { transaction: t },
    )
    ...
    try {
      ...
      if (isOwner && gig.freelancerId) {
        await sendNotificationToUser(
          gig.freelancerId,
          "Gig Cancelled",
          `The gig "${gig.title}" has been cancelled by the client. Reason: ${reason || "Not specified"}`,
          {
            type: "gig_cancelled",
            gigId: gig.id,
          },
      ...
      if (isFreelancer) {
        await sendNotificationToUser(
          gig.userId,
          "Gig Cancelled",
          `The gig "${gig.title}" has been cancelled by the freelancer. Reason: ${reason || "Not specified"}`,
          {
            type: "gig_cancelled",
            gigId: gig.id,
          },
