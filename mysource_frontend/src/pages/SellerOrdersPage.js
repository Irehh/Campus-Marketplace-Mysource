...
const SellerOrdersPage = () => {
  ...
  const fetchOrders = async () => {
    try {
      ...
      const response = await axios.get(`${REACT_APP_API_URL}/api/orders/seller?${params}`, {
  ...
  const updateDeliveryStatus = async (orderId, newStatus) => {
    ...
    try {
      ...
      const response = await axios.put(
        `${REACT_APP_API_URL}/api/orders/${orderId}/delivery-status`,
        { deliveryStatus: newStatus },
        { headers: { Authorization: `Bearer ${token}` } }
      );

      toast.success(response.data.message || "Delivery status updated successfully");
      await fetchOrders();
    } catch (error) {
    ...
    <div className="container mx-auto px-4 py-6">
      ...
      {orders.length === 0 ? (
        <EmptyState
          ...
          description={statusFilter ? `No orders with status "${statusFilter}"` : "You haven't received any orders yet."}
                ...
                <div className="p-4 border-b bg-gray-50">
                  <div className="flex justify-between items-start">
                    <div>
                      ...
                      <p className="text-sm text-gray-500">Customer: {order.buyer?.name || "Unknown"}</p>
                    </div>
