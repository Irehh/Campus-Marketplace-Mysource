...
const CartPage = () => {
  ...
  const fetchCart = async () => {
    try {
      setLoading(true)
      const response = await api.get("/cart?calculateFees=true")
  ...
  const createOrder = async () => {
    ...
    try {
      ...
      if (error.response?.status === 400 && errorMessage.includes("Insufficient balance")) {
        setTimeout(() => {
  ...
  return (
    <div className="container mx-auto px-4 py-6">
      <PageHeader
        title="Shopping Cart"
        subtitle={`${cartSummary.itemCount} item${cartSummary.itemCount !== 1 ? "s" : ""} in your cart`}
      />
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 space-y-6">
          {Object.entries(groupedItems).map(([sellerId, group]) => (
            <div key={sellerId} className="bg-white rounded-lg shadow-sm border">
              <div className="p-4 border-b bg-gray-50">
                <h3 className="font-medium text-gray-900">Sold by {group.seller?.name || "Unknown Seller"}</h3>
                  ...
                  <div key={item.id} className="p-4">
                    <div className="flex items-start space-x-4">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-sm font-medium text-primary mt-1">₦{Number(item.price).toLocaleString()}</p>
                      </div>
          ...
          <div className="flex justify-between items-center">
            ...
            <Link to="/products" className="text-primary hover:text-primary-600 text-sm font-medium">
              Continue Shopping
            </Link>
          </div>
        </div>
          ...
          <div className="bg-white rounded-lg shadow-sm border p-6 sticky top-6">
            ...
            <div className="mt-6 space-y-3">
              <button
                ...
                className="w-full bg-primary text-white py-3 px-4 rounded-md font-medium hover:bg-primary-600 disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center"
              >
                {creatingOrder ? (
                  <>
