  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (newImages.length > 0) {
        ...
        await axios.post(`/api/products/${id}/images`, formDataImages, {
          headers: {
    ...
    <div className="container mx-auto px-4 py-6 max-w-2xl">
      ...
      {product.isDisabled && (
        <div className="bg-red-50 border-l-4 border-red-500 p-4 mb-6 rounded-md">
          <p className="font-medium text-red-700">This product has been disabled by an admin.</p>
          {product.disabledReason && <p className="text-sm text-red-600 mt-1">Reason: {product.disabledReason}</p>}
          <p className="text-sm text-red-600 mt-1">
            This product is not visible to other users. Contact an admin if you believe this is a mistake.
          </p>
        </div>
      )}
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-4">
              ...
              {images.map((image) => (
                ...
                >
                  <img src={image.url || "/images/placeholder.png"} alt="Product" className="w-full h-full object-cover" />
                  <button
                    type="button"
                    onClick={() =>
          ...
          <div className="flex justify-between pt-4 border-t border-gray-200">
            <button type="button" onClick={handleDelete} className="flex items-center text-red-600 hover:text-red-800">
              <FiTrash2 className="mr-1" /> Delete Product
            </button>
