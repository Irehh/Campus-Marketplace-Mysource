...
// In src/pages/admin/AdminUsers.jsx
;
...
const AdminUsers = () => {
    ...
    const fetchUsers = async () => {
      try {
        const response = await axios.get(`${REACT_APP_API_URL}/api/admin/users`, {
          params: { search, campus: selectedCampus, page, limit: 10 },
  ...
  const handleMakeAdmin = async () => {
    try {
      await axios.post(
        `${REACT_APP_API_URL}/api/admin/admins`,
        { userId: selectedUser.id, campus: adminCampus },
        { headers: { Authorization: `Bearer ${token}` } },
      );
  ...
  const handleRemoveAdmin = async (userId) => {
    try {
      await axios.delete(`${REACT_APP_API_URL}/api/admin/admins/${userId}`, {
        headers: { Authorization: `Bearer ${token}` },
      });
  ...
  const handleRemoveUser = async () => {
    try {
      await axios.delete(`${REACT_APP_API_URL}/api/admin/users/${selectedUser.id}`, {
        headers: { Authorization: `Bearer ${token}` },
      });
          ...
          <div className="bg-white rounded-lg p-6 max-w-md w-full">
            ...
            <div className="bg-red-50 p-3 rounded-md mb-4">
              <p className="text-sm text-red-700">
                <span className="font-semibold">{selectedUser.name || selectedUser.email}</span> will be removed from the
                platform. All their products, businesses, and messages will also be deleted.
              </p>
            </div>
            <div className="flex justify-end space-x-2">
              <button
