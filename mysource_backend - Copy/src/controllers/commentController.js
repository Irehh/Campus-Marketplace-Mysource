...
export const getComments = async (req, res) => {
  ...
  try {
    const comments = await prisma.comment.findMany({
      where: {
        ...(itemType === "product" ? { productId: itemIdInt } : { businessId: itemIdInt }),
      },
      include: {
        user: {
          select: {
  ...
  try {
    ...
    if (itemType === "product") {
      const product = await prisma.product.findUnique({
        where: { id: itemIdInt }, // Use parsed integer
      });
      if (!product) {
    ...
    const comment = await prisma.comment.create({
      data: {
        content,
        userId,
        ...(itemType === "product" ? { productId: itemIdInt } : { businessId: itemIdInt }), // Use parsed integer
      },
      include: {
        user: {
          select: {
...
export const updateComment = async (req, res) => {
  ...
  try {
    ...
    const existingComment = await prisma.comment.findUnique({
      where: { id: commentId }, // Use parsed integer
    });
